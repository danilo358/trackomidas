...
export default function CartPage(){
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        async function autoCalc() {
            ...
            // geocoda o endereço do cliente selecionado
            const a = addresses.find(x => x._id === selectedAddr)
            const label = [a?.rua, a?.numero, a?.bairro, a?.cidade && `${a.cidade}/${a.uf}`].filter(Boolean).join(', ')
            if (!label) return
            const c = await geocodeAddress(label)
            setAddrCoord(c)
            // calcula distância se restaurante e cliente possuem coords
            if (restCoord && c) {
            setLoadingDist(true)
            try {
                const km = await drivingDistanceKm(restCoord, c)
  ...
  async function checkout(){
    ...
    await api.post('/orders', {
        restaurantId, itens: items, total,
      ...
      <div className="card grid gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-sm opacity-70">
            {loadingDist ? 'Calculando… ' : ''}Frete: R$ {freteFixo.toFixed(2)} + R$ {freteKm.toFixed(2)} × km
          </span>
        </div>
    ...
    <footer className="fixed bottom-0 inset-x-0 z-40 backdrop-blur bg-black/60 border-t border-white/10">
      <div className="mx-auto max-w-6xl px-4 h-16 flex items-center justify-between">
        <div className="text-sm opacity-80">
          ...
          Frete: <strong>R$ {(freteFixo + freteKm * (distanceKm || 0)).toFixed(2)}</strong>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-lg font-semibold">Total: R$ {(subtotal + (freteFixo + freteKm * (distanceKm || 0))).toFixed(2)}</span>
